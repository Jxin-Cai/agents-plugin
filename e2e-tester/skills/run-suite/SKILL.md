...
从 `$ARGUMENTS` 解析：套件名 → `.e2e-tests/shared/registry/suites.yaml` | 域名 → `.e2e-tests/shared/registry/{domain}.yaml` 全部非 stale | 标签/风险过滤 → 遍历匹配 | 显式列表 → 逐个查找。
`.e2e-tests/shared/quality-ledger.md` 存在时读取时序基线和环境陷阱。
