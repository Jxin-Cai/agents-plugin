...
读取 task.md、index.md、剧本、prep 方案、已有报告、quality-ledger（只提取与当前 domain 相关的时序基线、失败模式、环境陷阱条目）。
- readiness = BLOCKED → 停止执行
- real 依赖不可用且无降级 → BLOCKED

### 阶段 1: 资产检索与路径决策

查询 registry、asset-catalog、task 文件。匹配已有脚本（精确 → 模糊 → 辅助）。
...
逐 case 探索，拦截 API 调用链，收集证据，提炼接口知识。

### 阶段 3: 质量报告

> **条件加载**：此时读取 `references/report-template.md`。含 async/flaky 时才读 `references/async-and-flaky-guide.md`。

生成 `.e2e-tests/{domain}/reports/{date}/TS-{NNN}-run-{RRR}.md`。按 case 给出 PASS/FAIL/BLOCKED/SKIP。

### 阶段 4: 沉淀判断与索引回写

适合沉淀的条件：路径成立 + 证据完整 + 有 API 端点 + 准备可重复。
回写 index.md（报告路径、路径决策、case 结果、资产）。

### 阶段 4.5: 回写 quality-ledger

失败模式、时序基线、环境陷阱、依赖稳定性、flaky 治理。

### 阶段 5: 后续动作

`AskUserQuestion`（multiSelect）：补证据 / 重测 / 修改剧本 / 沉淀 / 结束。
