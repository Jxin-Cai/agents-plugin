#!/bin/bash
# PreCompact hook: 上下文压缩前保存关键合规任务状态到检查点
...

{
  ...
  echo ""
  echo "> 自动生成于 $(date '+%Y-%m-%d %H:%M:%S')，用于上下文压缩后恢复合规任务状态。"
  echo ""
  echo "## 最近任务"
  echo ""
  while IFS= read -r dir; do
    ...
    if [ -f "$state" ]; then
      ...
      laws=$(grep "^regulation_scope:" "$state" 2>/dev/null | head -1 | cut -d':' -f2- | xargs)
      risk=$(grep "^risk_focus:" "$state" 2>/dev/null | head -1 | cut -d':' -f2- | xargs)
    fi
    ...
    [ -z "$artifact" ] && artifact="-"
    echo "- ${name} | workflow=${workflow:-unknown} | next=${next:-unknown} | laws=${laws:-[]} | risk=${risk:-[]} | artifact=${artifact}"
