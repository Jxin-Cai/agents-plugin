...
在编排器内直接执行数据速查，不调用子技能。按以下四个维度逐一检查，每个维度输出不超过 5 行：

| 维度 | 速查内容 | 输出格式 |
|------|---------|---------|
| 营收概况 | 当月/当季营收总额、同比(YoY)和环比(MoM)增长率 | 数值 + 增减箭头 |
| 用户健康度 | MAU、新增用户数、次月留存率 | 数值 + 同环比变化 |
| 渠道效率 | Top 3 渠道的 ROAS 和 CPA | 排名表（渠道/ROAS/CPA） |
| 客户结构 | 活跃/沉睡/流失客户占比 | 百分比列表 |

输入：用户在 `$ARGUMENTS` 中提供的数据源或业务背景。
输出：生成 `_analytics/quick-scan-{日期}.md`，包含上述四维度速查结果和数据来源标注。
...
归因模型必须声明假设和局限性。
不可用相关性暗示因果性。
样本量不足时（n<30 或统计功效<0.8），必须在结论中标注置信度局限。
每个阶段完成后必须等待用户确认。
