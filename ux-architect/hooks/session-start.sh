...
if [ "$TASK_COUNT" -gt 0 ]; then
  ...
  while IFS= read -r dir; do
    ...
    ls "$dir"/ia/ia-*.md >/dev/null 2>&1 && artifacts="${artifacts}IA "
    ls "$dir"/flows/flow-*.md >/dev/null 2>&1 && artifacts="${artifacts}FLOW "
    ls "$dir"/interaction/audit-*.md >/dev/null 2>&1 && artifacts="${artifacts}AUD "
    [ -z "$artifacts" ] && artifacts="INIT"
    echo "  - ${name} | workflow=${workflow:-unknown} | next=${next:-unknown} | artifacts=[${artifacts}] | objective=${target:--}"
  done < <(ls -1dt "$WORKSPACE"/*/ 2>/dev/null | head -3)
fi

echo ""
echo "- 提示: 默认先走 /ux-architect:uxa 装配任务；如需恢复，可直接说明“继续上次 UX 架构任务”。"
echo ""
