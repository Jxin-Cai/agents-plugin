...
if [ "$TASK_COUNT" -gt 0 ]; then
  ...
  while IFS= read -r dir; do
    ...
    if [ -f "$state" ]; then
      ...
      next=$(grep "^next_step:" "$state" 2>/dev/null | head -1 | cut -d':' -f2- | xargs)
      target=$(grep "^objective:" "$state" 2>/dev/null | head -1 | cut -d':' -f2- | xargs)
      updated=$(grep "^updated_at:" "$state" 2>/dev/null | head -1 | cut -d':' -f2- | xargs)
    fi

    ls "$dir"/designs/*.md >/dev/null 2>&1 && artifacts="${artifacts}DES "
    ls "$dir"/metrics/*.md >/dev/null 2>&1 && artifacts="${artifacts}MET "
    ls "$dir"/results/*.md >/dev/null 2>&1 && artifacts="${artifacts}RES "
    [ -z "$artifacts" ] && artifacts="INIT"
    echo "  - ${name} | workflow=${workflow:-unknown} | stage=${stage:-unknown} | next=${next:-unknown} | artifacts=[${artifacts}] | objective=${target:--} | updated=${updated:--}"
  done < <(ls -1dt "$WORKSPACE"/*/ 2>/dev/null | head -3)
fi

echo ""
echo "- 提示: 默认先走 /experiment-tracker:et 装配任务；如需恢复，可直接说明“继续上次实验任务”。"
echo ""
