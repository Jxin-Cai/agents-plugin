...
if [ "$TASK_COUNT" -gt 0 ]; then
  ...
  while IFS= read -r dir; do
    ...
    ls "$dir"/components/component-review-*.md >/dev/null 2>&1 && artifacts="${artifacts}CMP "
    ls "$dir"/responsive/responsive-audit-*.md >/dev/null 2>&1 && artifacts="${artifacts}RWD "
    ls "$dir"/performance/performance-check-*.md >/dev/null 2>&1 && artifacts="${artifacts}PERF "
...
echo ""
echo "- 提示: 默认先走 /frontend-developer:fed 装配任务；如需恢复，可直接说明“继续上次前端任务”。"
echo ""
