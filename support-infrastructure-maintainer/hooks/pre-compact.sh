#!/bin/bash
# PreCompact hook: 上下文压缩前保存关键基础设施任务状态到检查点
...

{
  ...
  echo ""
  echo "> 自动生成于 $(date '+%Y-%m-%d %H:%M:%S')，用于上下文压缩后恢复基础设施任务状态。"
  echo ""
  echo "## 最近任务"
  echo ""
  while IFS= read -r dir; do
    ...
    if [ -f "$state" ]; then
      ...
      risk=$(grep "^budget_risk_compliance:" "$state" 2>/dev/null | head -1 | cut -d':' -f2- | xargs)
    fi

    artifact=$(find "$dir" -type f ! -path "*/meta/*" ! -path "*/context/*" 2>/dev/null | xargs ls -1t 2>/dev/null | head -1 | xargs -n1 basename 2>/dev/null)
    echo "- ${name} | workflow=${workflow:-unknown} | next=${next:-unknown} | risk=${risk:-[]} | artifact=${artifact:--}"
