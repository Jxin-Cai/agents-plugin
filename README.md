...
| `ask-buddy` | 本地优先的只读个人助理：渐进交互、分层记忆、飞书简报与可信研究 | 2.9.0 |

## ask-buddy

Ask Buddy 只读取工作区；唯一允许写入的位置是当前项目的 `.ask-buddy/` 记忆目录。它采用 profile、session、daily、curated memory、playbook、pending 六层纯 Markdown 状态模型：明确事实可保存，推断和自学习候选必须先经用户确认。内置零依赖、本地运行的 Memory MCP，提供分层检索、精确读取、状态检查、候选暂存与确定性审批；批准后分别形成 `PREF-NNN`、`MEM-NNN`、`PROC-NNN` 正式条目。启动 Hook 只注入 active profile、当前 checkpoint 和记忆健康摘要，长期记忆按需检索，不加载过期、被替代或符号链接内容。轻量 Stop hook 只提取用户明确表达的一条偏好或纠正，不归档完整聊天。另默认集成固定版本的 OneSearch MCP（DuckDuckGo），用于实时资料检索，并关闭对本机、回环和内网地址的抓取。
...
安装后可在 Claude Code 中用 `/mcp` 检查 `ask-buddy-memory`、`feishu-assistant-readonly` 和 `one-search`，用 `/hooks` 检查只读防护。Memory MCP 与飞书只读桥接均在本地启动；首次启用 MCP 时 Claude Code 会要求信任项目配置。

### 飞书接入

Ask Buddy 使用用户身份读取个人日历、任务、忙闲和邮箱。飞书只读桥接通过官方 CLI 提供这些读取工具，并提供项目级二维码绑定流程。插件不暴露创建日程、修改任务、发送邮件或删除数据的工具。
...
3. 安装官方 CLI 并初始化一次本机配置（app secret 由 CLI/keychain 管理，不写入项目文件）：
...
4. 重启 Claude Code 后，在当前项目中调用 `feishu_binding_preflight`；检查通过后调用 `feishu_binding_start`，扫描返回的二维码，再调用 `feishu_binding_complete`。绑定状态与 scope 元数据仅写入当前项目的 `.ask-buddy/.env`（权限 600）；设备码和 token 不会写入项目，二维码地址只作为本次绑定响应的临时字段返回。

也可用“今天有什么安排”“找个空闲时间”“整理我未完成的任务”“总结最近未读邮件”验证只读入口。邮箱和日程内容只用于当前回答，默认不写入长期记忆。
