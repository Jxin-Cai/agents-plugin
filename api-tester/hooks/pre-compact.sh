#!/bin/bash
# PreCompact hook: 上下文压缩前保存关键 API 测试任务状态到检查点
...

{
  ...
  echo ""
  echo "> 自动生成于 $(date '+%Y-%m-%d %H:%M:%S')，用于上下文压缩后恢复 API 测试任务状态。"
  echo ""
  echo "## 最近任务"
  echo ""
  while IFS= read -r dir; do
    ...
    ls "$dir"/contracts/contract-*.md >/dev/null 2>&1 && artifact="CON"
    ls "$dir"/integration/integration-test-plan-*.md >/dev/null 2>&1 && artifact="${artifact} INT"
    ls "$dir"/health/health-check-*.md >/dev/null 2>&1 && artifact="${artifact} HLT"
