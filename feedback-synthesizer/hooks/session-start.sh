...
if [ "$TASK_COUNT" -gt 0 ]; then
  ...
  while IFS= read -r dir; do
    ...
    ls "$dir"/raw-feedback/feedback-*.md >/dev/null 2>&1 && artifacts="${artifacts}RAW "
    ls "$dir"/analysis/sentiment-*.md >/dev/null 2>&1 && artifacts="${artifacts}ANA "
    ls "$dir"/insights/insights-*.md >/dev/null 2>&1 && artifacts="${artifacts}INS "
    [ -z "$artifacts" ] && artifacts="INIT"

    echo "  - ${name} | workflow=${workflow:-unknown} | stage=${stage:-unknown} | next=${next:-unknown} | artifacts=[${artifacts}] | updated=${updated:--}"
  done < <(ls -1dt "$WORKSPACE"/*/ 2>/dev/null | head -3)
fi

echo ""
echo "- 提示: 默认先走 /feedback-synthesizer:fs 装配任务；如需恢复，可直接说明“继续上次反馈任务”。"
echo ""
