...
echo "## 趋势研究工作台状态"
echo ""
task_count=$(find _trend-research -maxdepth 1 -mindepth 1 -type d 2>/dev/null | wc -l | tr -d ' ')
echo "- 任务数: ${task_count}"
if [ "$task_count" -gt 0 ]; then
  echo "- 最近任务:"
  ls -1t _trend-research/ 2>/dev/null | head -3 | while read d; do
    state="_trend-research/${d}/meta/state.md"
    if [ -f "$state" ]; then
